...
async function checkLocalDatabase(): Promise<boolean> {
  ...
  try {
    ...
    console.log(`✅ Banco local conectado (${result.length} time(s) encontrado)`);
...
async function migrateToHeroku(): Promise<boolean> {
  ...
  try {
    ...
    await execCommand(HEROKU_CMD, ["pg:reset", "DATABASE_URL", "-a", HEROKU_APP, "--confirm", HEROKU_APP]);

    // Importa o dump
    console.log("📤 Importando dados...");
    await execCommand(HEROKU_CMD, ["pg:psql", "-a", HEROKU_APP, "-f", DUMP_FILE]);
...
async function verifyMigration(): Promise<boolean> {
  ...
  try {
    ...
    const counts = await execCommand(HEROKU_CMD, [
      "pg:psql",
      "-a", HEROKU_APP,
      "-c", "SELECT 'Teams' as tabela, COUNT(*) as total FROM teams UNION ALL SELECT 'Tournaments', COUNT(*) FROM tournaments UNION ALL SELECT 'Matches', COUNT(*) FROM matches UNION ALL SELECT 'Users', COUNT(*) FROM \"user\";",
    ]);
