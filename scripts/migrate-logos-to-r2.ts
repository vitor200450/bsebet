    ...
    for (const team of batch) {
      try {
        ...
        if (!isBase64DataUrl(team.logoUrl)) {
          console.log(`  ⚠️  Time ${team.id} (${team.name}): Formato inválido, pulando`);
          result.skipped++;
          continue;
        }
...
async function migrateTournamentLogos(): Promise<MigrationResult> {
  ...
  console.log(`📊 Encontrados ${tournamentsWithLogos.length} torneios com logos`);
    ...
    for (const tournament of batch) {
      try {
        ...
        if (tournament.logoUrl.startsWith("http")) {
          console.log(`  ⏭️  Torneio ${tournament.id} (${tournament.name}): Já é URL externa`);
          result.skipped++;
          continue;
        }
        ...
        if (!isBase64DataUrl(tournament.logoUrl)) {
          console.log(`  ⚠️  Torneio ${tournament.id} (${tournament.name}): Formato inválido, pulando`);
          result.skipped++;
          continue;
        }
        ...
        console.log(`  ✅ Torneio ${tournament.id} (${tournament.name}): Migrado com sucesso`);
        console.log(`     URL: ${publicUrl}`);
        result.success++;
      } catch (error) {
