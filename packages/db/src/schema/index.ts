...
// --- 4. TABELA: AJUSTES MANUAIS DE PONTOS ---
export const pointAdjustments = pgTable(
	"point_adjustments",
	{
		...
		tournamentId: integer("tournament_id")
			.notNull()
			.references(() => tournaments.id),
		matchId: integer("match_id").references(() => matches.id), // opcional - qual partida causou o problema
		points: integer("points").notNull(), // pode ser positivo (compensação) ou negativo (correção)
		reason: text("reason").notNull(), // justificativa obrigatória
		adminId: text("admin_id")
			.notNull()
			.references(() => user.id), // quem fez o ajuste
		createdAt: timestamp("created_at").defaultNow().notNull(),
		isRecoveryCompensation: boolean("is_recovery_compensation").default(false), // flag específica para recovery bets
	},
	(table) => ({
		// Indexes for common query patterns
		userIdIdx: index("pa_user_id_idx").on(table.userId),
		tournamentIdIdx: index("pa_tournament_id_idx").on(table.tournamentId),
		adminIdIdx: index("pa_admin_id_idx").on(table.adminId),
		createdAtIdx: index("pa_created_at_idx").on(table.createdAt),
		// Composite index for filtered queries
		userTournamentIdx: index("pa_user_tournament_idx").on(
			table.userId,
			table.tournamentId,
		),
	}),
...
export const pointAdjustmentsRelations = relations(
	...
	({ one }) => ({
		...
		tournament: one(tournaments, {
			fields: [pointAdjustments.tournamentId],
			references: [tournaments.id],
		}),
		match: one(matches, {
			fields: [pointAdjustments.matchId],
			references: [matches.id],
		}),
		admin: one(user, {
