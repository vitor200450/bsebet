...
const getLeaderboardFn = createServerFn({
	...
	const previousMonthRanks: Map<string, number> = new Map();
	const globalRanks: Map<string, number> = new Map();
	...
	if (tournamentId) {
		...
		if (mostImportantMatch.length > 0) {
			...
			mostImportantMatchWinners = new Set(correctBets.map((b) => b.userId));
		}
	...
	const sortedRows = rows
		.map((row) => ({
			...
			bestPreviousMonthResult: tournamentId
				...
				: undefined,
			globalRank: tournamentId
				? (globalRanks.get(row.userId) ?? null)
				: undefined,
		}))
		.sort((a, b) => {
			...
			if (tournamentId) {
				...
				const prevMonthA =
					a.bestPreviousMonthResult ?? Number.POSITIVE_INFINITY;
				const prevMonthB =
					b.bestPreviousMonthResult ?? Number.POSITIVE_INFINITY;
				if (prevMonthA !== prevMonthB) return prevMonthA - prevMonthB;

				// 7° Global Rank (ascending - 1st is better than 10th)
				const globalRankA = a.globalRank ?? Number.POSITIVE_INFINITY;
				const globalRankB = b.globalRank ?? Number.POSITIVE_INFINITY;
...
const getLeaderboardTournamentsFn = createServerFn({
	...
	const result = await db
		...
		.innerJoin(bets, eq(bets.matchId, matches.id))
		.where(
			or(
