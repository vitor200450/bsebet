		...
		img.onload = () => {
			...
			try {
				...
				if (sortedColors.length === 0) {
					console.warn(
						"[ColorExtractor] No valid colors found, using defaults",
					);
					resolve({ primary: "#2e5cff", secondary: "#ff2e2e" });
					return;
				}
				...
				const primary = sortedColors[0]
					? rgbToHex(
							...(sortedColors[0][0].split(",").map(Number) as [
								number,
								number,
								number,
							]),
				...
				const secondary = sortedColors[1]
					? rgbToHex(
							...(sortedColors[1][0].split(",").map(Number) as [
								number,
								number,
								number,
							]),
				...
				console.log(
					"[ColorExtractor] Final colors - Primary:",
					primary,
					"Secondary:",
					secondary,
				);
 ...
 */
export function getIntermediateColor(color1: string, color2: string): string {
